...
%     CVPRim = imresize(CVPRim, [size(im,1) size(im,2)]);
    ...
    %time_cost = toc(t);
    %fprintf( 'time cost for saliency HDCT1 %.3f\n', time_cost );
    ...
    %time_cost = toc(t);
    %fprintf( 'time cost for saliency HDCT2 %.3f\n', time_cost );
    ...
    %time_cost = toc(t);
    %fprintf( 'time cost for saliency HDCT3 %.3f\n', time_cost );
    %% Colors
    ...
    %time_cost = toc(t);
    %%fprintf( 'time cost for saliency HDCT4 %.3f\n', time_cost );

    final_true = mat2gray(exp(0.5.*final) + exp(0.5*ress));

    imwrite(final_true, ['result_stanford/' images(zz).name(1:end-4) '.png'], 'png');
    %time_cost = toc(t);
    %fprintf( 'time cost for saliency HDCT %.3f\n', time_cost );
end
fclose(filepw);